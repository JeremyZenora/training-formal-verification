...
pragma solidity ^0.8.3;  //@audit-low in version 0.8.3 there are no custom errors, so this version would not even compile.
...
library MathMasters {
    ...
    function mulWad(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            ...
            if mul(y, gt(x, div(not(0), y))) {
                //@audit-low it will revert with blank message
                //@audit-info why are you overwriting the free memory pointer? 
                //@audit-info wrong error selector!
    ...
    function mulWadUp(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            // Equivalent to `require(y == 0 || x <= type(uint256).max / y)`.
            if mul(y, gt(x, div(not(0), y))) {
                //@audit-same thing, wrong error selector, overwriting memory pointer, reverting blank message
                mstore(0x40, 0xbac65e5b) // `MathMasters__MulWadFailed()`.
                revert(0x1c, 0x04)
            }           
            
            //@audit-high this line which adds "1" to x, is wrong, messes up logic
            if iszero(sub(div(add(z, x), y), 1)) { x := add(x, 1) }    // isZero? ((0 + x) / y) - 1) it will x = x + 1
            z := add(iszero(iszero(mod(mul(x, y), WAD))), div(mul(x, y), WAD)) // (iszero(iszero(((x * y) % 1e18))) + ((x * y) / 1e18)
        }
    }
